...
import {
  updateArticleStatus, updateArticleCommentary, createDigest, updateDigest,
  assignArticlesToDigest, getDigests, getDigest,
} from '../db/index.js';
...
const CATEGORIES = new Set(['production', 'print-business', 'design-trends', 'furniture-market', 'new-niches', 'other']);
const SIGNAL_TYPES = new Set([
  'digital-printing', 'on-demand', 'equipment-finishing', 'textile-chemistry',
  'design-trend', 'designer-case', 'furniture-market', 'installation-art', 'automation', 'other',
]);
...
const CATEGORY_ORDER = ['production', 'print-business', 'design-trends', 'furniture-market', 'new-niches', 'other'];
...
function cleanText(value, maxLength = 1000) {
  return String(value || '')
    ...
    .replace(/\s+/g, ' ').replace(/texturalab/gi, 'TexturaLab').trim().slice(0, maxLength);
}

function extractJson(rawText) {
  const cleaned = String(rawText || '').replace(/<think>[\s\S]*?<\/think>/gi, '').replace(/```json/gi, '').replace(/```/g, '').trim();
...
const CLUSTER_FAMILIES = {
  'print-on-demand': new Set(['on-demand', 'digital-printing']),
  'finishing-chemistry': new Set(['equipment-finishing', 'textile-chemistry']),
  'design-trends': new Set(['design-trend']),
  'process-automation': new Set(['automation']),
  'furniture-market': new Set(['furniture-market']),
};
function familyForTypes(types) {
  for (const [family, allowed] of Object.entries(CLUSTER_FAMILIES)) if (types.every((type) => allowed.has(type))) return family;
  return null;
}
...
function normalizeSynthesis(value, candidateMap) {
  ...
  const clusters = (Array.isArray(value.clusters) ? value.clusters : []).map((cluster) => {
    const sourceIds = [...new Set((Array.isArray(cluster.source_ids) ? cluster.source_ids : []).map(String).filter((id) => candidateMap.has(id)))];
    const sourceEntries = sourceIds.map((id) => candidateMap.get(id));
    const family = familyForTypes(sourceEntries.map((entry) => entry.card.signal_type));
    return {
      title: cleanText(cluster.title, 180), finding: cleanText(cluster.finding, 700), why_it_matters: cleanText(cluster.why_it_matters, 700),
      action: cleanText(cluster.action, 500), confidence: cleanText(cluster.confidence, 30).toLowerCase(), source_ids: sourceIds, family,
    };
  }).filter((cluster) => {
    if (!cluster.title || !cluster.finding || !cluster.why_it_matters || !cluster.action || !cluster.source_ids.length || !cluster.family) return false;
    const sourceEntries = cluster.source_ids.map((id) => candidateMap.get(id));
    if (sourceEntries.length === 1) return sourceEntries[0].card.relevance_score >= 70 && ['print-on-demand', 'furniture-market'].includes(cluster.family);
    return sourceEntries.some((entry) => entry.card.relevance_score >= MIN_RELEVANCE_SCORE);
  }).slice(0, MAX_CLUSTERS);
  if (!clusters.length) throw new Error('Synthesis не содержит валидных тематических кластеров');
  return { clusters };
}
...
async function synthesizeEntries(candidates, config, log) {
  ...
  const compact = candidates.map(({ article, card }) => ({ id: String(article.id), source: (article.source || '').replace(/^rss:/, ''), title: article.title, score: card.relevance_score, signal_type: card.signal_type, category: card.category, facts: card.facts, summary: card.summary }));
  const system = ['/no_think', 'Ты — старший аналитик TexturaLab.', 'Работай только с переданными фактами. Не добавляй факты, рынки и применения.'].join('\n');
  const baseUserMessage = [
    'Собери 2-5 аналитических кластеров из материалов с score >= 40.',
    'Кластер — общий КОНКРЕТНЫЙ механизм, а не общие слова вроде эффективность, устойчивость, инновации, современные технологии или цифровизация.',
    'Допустимые семейства: (1) on-demand + digital-printing; (2) equipment-finishing + textile-chemistry; (3) design-trend + design-trend; (4) automation + automation; (5) furniture-market + furniture-market.',
    'Не смешивай automation с finishing/chemistry только потому, что всё относится к производству.',
    'Designer-case, installation-art и other не используй для главных выводов.',
    'Пограничный материал 40-54 допускается только в кластере 2+ материалов, где есть хотя бы один источник 55+.',
    '',
    'ДЕЙСТВИЕ ДОЛЖНО БЫТЬ НАПРАВЛЕНО НА БИЗНЕС TEXTURALAB, А НЕ НА ПОКУПКУ/ТЕСТ ОБОРУДОВАНИЯ ГЕРОЯ СТАТЬИ.',
    'Формат действия: сигнал -> гипотеза TexturaLab -> дешёвый следующий тест.',
    'Предпочитай: проверить коммерческое предложение, ассортимент/капсулу, экономику коротких серий, применимость процесса на имеющейся линии, запросить технические данные для сравнения.',
    'Не предлагай покупать, прототипировать или внедрять Kornit/Brückner/ELİAR/другое чужое оборудование, если источники прямо не обосновывают такую инвестицию.',
    'Для finishing + chemistry допустимый вопрос — применимость к отделке/пропитке/термофиксации полиэстера на существующей линии; не утверждай применимость как факт.',
    'Для design-trend действие — небольшой тест дизайнов/капсулы на своей аудитории, а не заявление о гарантированном спросе.',
    '', 'Верни только JSON:',
    '{"clusters":[{"title":"...","finding":"что совместно подтверждают источники","why_it_matters":"конкретное значение для TexturaLab","action":"гипотеза TexturaLab и дешёвый следующий тест","confidence":"низкая | средняя","source_ids":["id1","id2"]}]}',
    '', 'МАТЕРИАЛЫ:', JSON.stringify(compact),
  ].join('\n');
  let inputTokens = 0, outputTokens = 0, lastError = null;
  for (let attempt = 1; attempt <= SYNTHESIS_ATTEMPTS; attempt += 1) {
    const user = attempt === 1 ? baseUserMessage : `${baseUserMessage}\nПредыдущий JSON не прошёл проверку: ${lastError?.message}. Не смешивай несовместимые signal_type.`;
    const response = await callModel(config, { system, user, maxTokens: 1600 });
    inputTokens += response.inputTokens; outputTokens += response.outputTokens;
    try { return { synthesis: normalizeSynthesis(extractJson(response.text), candidateMap), inputTokens, outputTokens }; }
    catch (error) { lastError = error; log.push(`Synthesis invalid JSON attempt ${attempt}: ${error.message}`); }
  }
...
function renderEntry({ article, card }) {
  ...
  return [`#### ${card.title_ru}`, '', `**Полезность:** ${card.relevance_score}/100 · тип ${card.signal_type} · уверенность ${card.confidence}`, '', '**Факты из RSS**', factLines, '', '**Нейтральное резюме**', card.summary, '', `**Почему такой балл:** ${card.score_reason}`, '', `Источник: ${(article.source || '').replace(/^rss:/, '').replace(/^www\./, '')}`, `Читать оригинал: ${article.url}`].join('\n');
}
...
function buildDigest(entries, synthesis, config, skippedCount = 0) {
  ...
  const clusterSections = synthesis.clusters.map((cluster, index) => {
    ...
    return [`### ${index + 1}. ${cluster.title}`, '', `**Вывод:** ${cluster.finding}`, '', `**Почему это важно TexturaLab:** ${cluster.why_it_matters}`, '', `**Действие:** ${cluster.action}`, '', `**Уверенность:** ${cluster.confidence || 'средняя'}`, '', '**Основание**', sourceLines].join('\n');
  });
  const groupedSections = [];
  for (const category of CATEGORY_ORDER) {
    const categoryEntries = sorted.filter(({ card }) => card.category === category);
    if (!categoryEntries.length) continue;
    groupedSections.push([`### ${CATEGORY_LABELS[category]}`, '', ...categoryEntries.flatMap((entry, index) => index === categoryEntries.length - 1 ? [renderEntry(entry)] : [renderEntry(entry), '', '---', ''])].join('\n'));
  }
  const actions = [], seen = new Set();
  for (const cluster of synthesis.clusters) {
    const action = cleanText(cluster.action, 500), key = action.toLowerCase();
    if (!action || seen.has(key)) continue;
    seen.add(key); actions.push(action); if (actions.length >= MAX_ACTIONS) break;
  }
...
export async function generateDigest(db, articles, config) {
  const log = []; let totalInputTokens = 0, totalOutputTokens = 0;
  ...
  if (!analyzedEntries.length) throw new Error('No valid article analyses — cannot assemble digest');
  ...
  if (!candidates.length) throw new Error(`No articles scored at least ${SYNTHESIS_FLOOR}`);
  const synthesisResult = await synthesizeEntries(candidates, config, log);
  totalInputTokens += synthesisResult.inputTokens; totalOutputTokens += synthesisResult.outputTokens;
  const clusterIds = new Set(synthesisResult.synthesis.clusters.flatMap((cluster) => cluster.source_ids));
  const entries = analyzedEntries.filter(({ article, card }) => card.relevance_score >= MIN_RELEVANCE_SCORE || (card.relevance_score >= SYNTHESIS_FLOOR && clusterIds.has(String(article.id)))).sort((a, b) => b.card.relevance_score - a.card.relevance_score);
  const finalIds = new Set(entries.map(({ article }) => String(article.id)));
  const finalSynthesis = { clusters: synthesisResult.synthesis.clusters.map((cluster) => ({ ...cluster, source_ids: cluster.source_ids.filter((id) => finalIds.has(id)) })).filter((cluster) => cluster.source_ids.length > 0) };
  const skippedEntries = analyzedEntries.filter(({ article }) => !finalIds.has(String(article.id)));
  for (const { article, card } of skippedEntries) { log.push(`Filtered article ${article.id}: score=${card.relevance_score}, type=${card.signal_type}`); updateArticleStatus(article.id, 'ignored'); }
  if (!entries.length || !finalSynthesis.clusters.length) throw new Error('No supported clusters remained after synthesis');
  ...
  const saved = getDigest(digestId); const digestOk = saved && typeof saved.content === 'string' && saved.content.length > 100 && saved.content.includes('#ДайджестTexturaLab');
  if (!digestOk) log.push('Skipping source cleanup: digest not confirmed valid');
  else if (config.telegramBotToken) {
    const { deleteTelegramMessage } = await import('./telegram-bot.js'); const seen = new Set(); let deleted = 0, failed = 0;
    for (const { article } of entries) {
      if (!article.source_chat_id || !article.source_message_id) continue;
      const key = `${article.source_chat_id}:${article.source_message_id}`; if (seen.has(key)) continue; seen.add(key);
      try { const ok = await deleteTelegramMessage(config.telegramBotToken, article.source_chat_id, Number(article.source_message_id)); if (ok) deleted += 1; else failed += 1; } catch { failed += 1; }
    }
...
function saveDigestToFile(date, content) {
  const __dirname = dirname(fileURLToPath(import.meta.url)); const outputDir = join(__dirname, '../../output'); mkdirSync(outputDir, { recursive: true });
  const existing = getDigests().filter((digest) => digest.date === date); const part = existing.length || 1;
  const filePath = join(outputDir, `digest_${date}_part${part}.txt`); writeFileSync(filePath, content, 'utf-8'); console.log(`[digest-generator] Saved digest to ${filePath}`); return filePath;
}
